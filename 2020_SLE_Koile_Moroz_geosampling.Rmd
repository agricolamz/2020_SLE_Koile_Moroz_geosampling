...
* proportion of villages: 0.05, 0.06, $\dots$, 0.89, 0.9
...
* We introduced a geographical sampling algorithm for detecting variation in languages, based on different clustering methods
* We tested our algorithm against random sampling in data simulated with different geographical distributions and numbers of observations
* We have found that our algorithm outperforms random sampling on simulated data in the cases where an underlying geographical structure is present, and performs equally well as it when variation is uniformly distributed across space
* We applied our algorithm to real data from Circassian languages
* We have found that our algorithm outperforms random sampling on real data for the small sample proportions, but hirarchical clustering has bad perfomance on larger sample proportions on those data
